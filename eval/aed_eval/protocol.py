...
def extract_source(text: str) -> str | None:
    ...
    Returns the LAST fenced block: models commonly quote the failing
    snippet before presenting the corrected file, and taking the first block
    would grade the wrong artifact.

    Returns None when there is no fenced block at all. That is a real
    emission failure and is recorded as one — never silently treated as the
    whole response body, which would let prose be graded as source.
    """
...
def run_trial(config: TrialConfig, model, gate, seed: int, workdir=None) -> TrialResult:
    ...
    for cycle in range(config.max_cycles):
        ...
        source = extract_source(response.text)
...
def run_arm(config: TrialConfig, model, gate_factory, seeds: list[int], workdir=None) -> dict:
    ...
    """
    trials = [run_trial(config, model, gate_factory(), seed, workdir) for seed in seeds]
...
def pair_discordance(arm_a: dict, arm_b: dict) -> tuple[int, int]:
    ...
    the arms did not run the same seeds, because pairing them would then be
    meaningless — and a silently unpaired "paired" test would report a
